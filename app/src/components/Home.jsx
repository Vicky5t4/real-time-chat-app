...
const Home = () => {
  ...
  useEffect(() => {
    if (selectedUser) {
      const fetchMessages = async () => {
        try {
          const res = await axios.get(`${BACKEND_URL}/api/auth/messages/${userId}/${selectedUser._id}`);
  ...
  const deleteAllMessages = async () => {
    ...
    try {
      await axios.delete(`${BACKEND_URL}/api/auth/messages/${userId}/${selectedUser._id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
      setMessages([]);
    } catch (err) {
    ...
    <div className='home-container'>
        ...
        <ul className="user-list">
          {users.filter((user) => user._id !== userId).map((user) => (
            <li className='user-item' key={user._id} onClick={() => setSelectedUser(user)}>
              <img src={`${BACKEND_URL}/${user.image}`} className='user-image' />
      ...
      <div className='content'>
        <div className='profile-section'>
          <img src={`${BACKEND_URL}/${image}`} className='profile-image' />
            ...
            <div className='messages'>
              {messages
                ...
                .map((msg, index) => (
                  ...
                  >
                    <img
                      src={`${BACKEND_URL}/${msg.from === userId ? image : selectedUser.image}`}
                    />
                    <p>{msg.content}</p>
